...
import { DayOfWeek, ScheduleEvent } from "@/@types/shared.types"
...
export default function ScheduleClientPage({
  ...
  const watchedValues = useWatch({ control }) as EventFormData
  ...
  const eventsByDay = dayOrder.reduce(
    ...
    {} as Record<DayOfWeek, ScheduleEvent[]>
  )
  ...
  async function onSubmit(data: EventFormData) {
    ...
    const result = editingEvent
      ? await updateScheduleEventAction(churchId, editingEvent.id, data)
      : await createScheduleEventAction(churchId, data)
    ...
    if (editingEvent) {
      setEvents((current) =>
        current.map((event) => (event.id === editingEvent.id ? result.data! : event))
      )
  ...
  async function deleteEvent(id: string) {
    const result = await deleteScheduleEventAction(churchId, id)
    if (!result.success) {
    ...
    >
      <motion.div variants={item}>
        <PageHeader
          ...
          description="Configure cultos, reunioes e eventos com preview e feedback imediato para a equipe."
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Select
                      value={watchedValues.dayOfWeek || "sunday"}
                      onValueChange={(value) =>
                        setValue("dayOfWeek", value as ScheduleEvent["dayOfWeek"])
                      }
                    >
                    ...
                    <div className="flex items-center gap-3 rounded-xl bg-card p-3 shadow-sm">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {dayOfWeekLabels[watchedValues.dayOfWeek || "sunday"]} - {watchedValues.time || "00:00"}
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    {events.slice(0, 4).map((event) => (
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {dayOfWeekLabels[event.dayOfWeek]} - {event.time}
                          </p>
                        </div>
                      </div>
                    ))}
      ...
      <motion.div variants={item} className="space-y-4">
        ...
        {events.length === 0 ? (
          <EmptyState
            ...
            description="Crie o primeiro evento para publicar a agenda da igreja e destravar os demais fluxos dependentes."
            action={
              <Button className="rounded-2xl bg-primary" onClick={() => setIsModalOpen(true)}>
                <Plus className="mr-2 h-4 w-4" />
