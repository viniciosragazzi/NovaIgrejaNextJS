...
import { Eye, EyeOff, Mail, Lock, User, ArrowRight, ChevronLeft, AlertCircle } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { cn } from "@/lib/utils";
import { useRouter } from "next/navigation";

// --- IMPORTANTE: Certifique-se de que estas actions existam ---
import { loginChurchAction, registerMemberAction } from "./actions";


type AuthMode = "login" | "register";

export function AuthForm({ church }: { church: any }) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <motion.div
        ...
        className="flex flex-col items-center gap-4 px-6 pb-6 pt-8"
      >
      ...
      <div className="mx-6 mb-6 flex rounded-full bg-muted p-1">
        <button
          ...
          className={cn(
            "relative flex-1 rounded-full py-3 text-sm font-medium transition-colors",
          ...
          >
            ...
            <div>
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  className="h-14 rounded-2xl border-0 bg-card pl-12 pr-12 text-base ring-1 ring-border transition-all focus:ring-2 focus:ring-primary"
                  required
                />
                <button
                  type="button"
            ...
            <div className="mt-4">
              <Button
                type="submit"
                disabled={isLoading}
                className="relative h-14 w-full rounded-2xl bg-primary text-base font-semibold text-primary-foreground shadow-lg shadow-primary/20 transition-all hover:bg-primary/90 disabled:opacity-70"
              >
                {isLoading ? (
                  <div className="flex items-center gap-2">
                    <motion.div
      ...
      >
        ...
        >
          {mode === "login" ? (
            ...
            <>Já tem uma conta? <span className="font-semibold text-foreground underline decoration-primary/30 underline-offset-4">Faça login</span></>
          )}
        </button>
      </motion.div>
    </div>
